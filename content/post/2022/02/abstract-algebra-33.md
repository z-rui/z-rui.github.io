...
1. 设 \\(a(x)=2x^5-5x^4+5 \in \Q\\)。取 \\(p=5\\)，根据 Eisenstein 判据可知 \\(a(x)\\) 不可约。设 \\(c\\) 是 \\(p(x)\\) 的一个根，那么 \\([\Q(c):\Q]=5\\)。设 \\(a(x)\\) 的分裂域为 \\(K\\)，那么 \\(5\mid [K:\Q]\\)。根据第三十二章习题 F.4--F.6 可知 \\(K=S_5\\)。根据本章定理 4，它是不可解群。因此 \\(a(x)\\) 没有根式解。
...
下面证明 \\(B\\) 中含有所有形如 \\(xyx^{-1}y^{-1}\\) 的元素，其中 \\(x,y\in A_4\\)。如果 \\(x=y\\) 则 \\(xyx^{-1}y^{-1}=\epsilon\\) 显然在 \\(B\\) 中。下面考虑 \\(x\ne y\\) 的情况。分类讨论：
...
**证明** 根据已知条件可知 \\(d^p=a\\)。设 \\(k\\) 是整数，不难验证
\\[(d\omega^k)^p = d^p(\omega^p)^k = d^p = a\\]
因此 \\(d\omega^k\\) 也是 \\(x^p-a\\) 的根。其中不相同的值共有 \\(p\\) 个：
...
**答** 因为 \\(m = \deg p(x) < \deg a(x) = p\\)，而 \\(p\\) 是质数，所以 \\(m\\) 和 \\(p\\) 互质，即 \\(\gcd(m,p)=1\\)。根据 Bézout 定理可知存在整数 \\(s,t\\) 使得 \\(sm+tp=1\\)。
...
**证明** 设 \\(H\lhd G\\) 是正规真子群。因为 \\(G\\) 是有限群，所以它只有有限个正规真子群，设它们是 \\(H_1,\ldots,H_n\\)。不失一般性，设 \\(|H_1|\ge\ldots\ge|H_n|\\)。并设集合
...
**证明** 设 \\(K=f^{-1}(J)\\)。那么对任意 \\(x\in K \iff f(x)\in J\\)。
...
所以 \\(\hat{\mathscr J}\\) 总是平凡子群。由此可知 \\(G/K\\) 没有非平凡的正规子群。

---

__D.5__ 如果 Abel 群 \\(G\\) 没有非平凡子群，那么 \\(G\\) 一定是质数阶循环群。（否则，选择某个 \\(a\in G\\) 使得 \\(\langle a\rangle\\) 是 \\(G\\) 的真子群。）

**说明** 这里的假定应该是 \\(G\\) 是有限阶非平凡群。

**证明** 如果 \\(G\\) 不是质数阶循环群，那么它的阶不是质数。设它的阶是 \\(n\\)，且质数 \\(p\mid n\\)。那么，根据 Cauchy 定理，存在阶等于 \\(p\\) 的元素 \\(a\in G\\)。于是，\\(\langle a\rangle\\) 是 \\(G\\) 的非平凡子群。这就证明了要证的命题的逆否命题。

---

__D.6__ 如果 \\(H\unlhd K\unlhd G\\)，那么 \\(G/K\\) 是 \\(G/H\\) 的同态像。

**证明** 设 \\(f\colon G/H\to G/K\\) 定义为 \\(f(Hx)=f(Kx)\\)。只须证明它是良定义的满同态即可。
...
根据 D.1，存在极大正规子群 \\(H_{(1)} \lhd G\\) 使得 \\(H\subseteq H_{(1)}\\)。因为 \\(H\lhd G\\)，所以 \\(H\unlhd H_{(1)}\\)。如果 \\(H\ne H_{(1)}\\)，那么按照同样的推理，又可以得到极大正规子群 \\(H_{(2)}\\)，满足 \\(H\unlhd H_{(2)}\lhd H_{(1)}\lhd G\\)。不断重复这一过程，因为每次得到的极大正规子群的阶都严格地递减，因此这个过程不可能无限地进行下去。假设进行 \\(q\\) 次后有 \\(H=H_{(q)}\\)。那么有
\\[H=H_{(q)}\lhd H_{(q-1)}\lhd\cdots\lhd H_{(1)}\lhd G\\]
把 \\(H_{(i)}\\) 重命名为 \\(H_{q-i}\\) 并规定 \\(H_q=G\\)，就得到序列
\\[H=H_0\lhd H_1\lhd\cdots\lhd H_{q-1}\lhd H_{q} = G\\]

对于任意 \\(0\le i < q\\)，因为 \\(H\unlhd H_i\lhd H_{i+1} \unlhd G\\)，所以根据 D.6 可知 \\(H_{i+1}/H_i\\) 是 \\(H_{i+1}/H\\) 的同态像，而且 \\(H_{i+1}/H\\) 是 \\(G/H\\) 的同态像。由此可知 \\(H_{i+1}/H_i\\) 是 \\(G/H\\) 的同态像。而已知 \\(G/K\\) 是 Abel 群，所以 \\(H_{i+1}/H_i\\) 也是 Abel 群。按照之前的构造法，\\(H_i\\) 是 \\(H_{i+1}\\) 的极大正规子群，根据 D.4 可知，\\(H_{i+1}/H_i\\) 没有非平凡正规子群。因为 Abel 群的子群都是正规子群，所以 \\(H_{i+1}/H_i\\) 没有非平凡子群。根据 D.5 可知，\\(H_{i+1}/H_i\\) 是质数阶循环群。
...
**注** 稍微详细地解释一下：本章正文对可解群的定义如同习题 B 所述：存在正规序列
\\[\\{e\\}=H_0^\* \unlhd H_1^\* \unlhd \cdots \unlhd H_n^\* = G\\]
对于每一组 \\(H_i \unlhd H_{i+1}\\)，因为 \\(H_{i+1}/H_i\\) 是 Abel 群，所以可以用 D.7 的方法往里填满所有的中间子群，使得每一组相邻的正规子群都是极大正规子群。对它们重新编号，得
\\[\\{e\\}=H_0 \lhd H_1 \lhd \cdots \lhd H_n = G\\]
其中 \\(H_i\\) 是 \\(H_{i+1}\\) 的极大正规子群。于是得到 \\(H_{i+1}/H_i\\) 是质数阶的循环群。

反过来，如果已经构造出了以上序列，那么自然 \\(H_{i+1}/H_{i}\\) 都是 Abel 群，因此也满足习题 B 中正规序列的定义。

## E. 如果 Gal(K:F) 可解，那么 K 是 F 的根式扩张

设 \\(K\\) 是 \\(F\\) 的有限扩张，其中 \\(K\\) 是 \\(F\\) 上的分裂域，并且 \\(\DeclareMathOperator\Gal{Gal}G=\Gal(K:F)\\) 是可解群。根据本章正文中的说明，我们假定 \\(F\\) 包含所必需的单位根。根据习题 D，设 \\(H_0,\ldots,H_n\\) 是 \\(G\\) 的可解序列，其中每个商群 \\(H_{i+1}/H_i\\) 都是质数阶循环群。对于任意 \\(i=1,\ldots n\\)，设 \\(F_i\\) 是 \\(H_i\\) 的固定域。
...
**证明** 设 \\(a(x)=x^p-c^p\\)。代入易知 \\(c\\) 是 \\(a(x)\\) 的根。事实上，\\(a(x)\\) 的所有根为 \\(\omega^k c\\)，而 \\(\pi\\) 移动 \\(a(x)\\) 的根 \\(\omega^k c\\)，所以这些根都不在固定域（即 \\(F_{i+1}\\)）内。也就是说，\\(a(x)\\) 在 \\(F_{i+1}\\) 上没有根。根据习题 C 可知，\\(a(x)\\) 在 \\(F_{i+1}\\) 上不可约。因此 \\(a(x)\\) 是 \\(c\\) 的最小多项式。
